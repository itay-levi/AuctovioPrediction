...
export async function createSimulation(
  ...
  // Full multi-phase panel runs for every tier. FREE-tier merchants see the
  // result blurred under glass until they pay $4.99 to unlock — the data
  // is the same, only the visual mask changes.
...
/**
 * Marks a simulation unlocked after the merchant has paid the $4.99 charge.
 * The full report data is already in the row (we run the full panel for
 * everyone regardless of tier) so this just flips the gate; the UI removes
 * the blur on next render.
 */
export async function unlockSimulation(args: {
  simulationId: string;
  chargeId: string;
  shopDomain: string;
}): Promise<void> {
  const { simulationId, chargeId, shopDomain } = args;
  ...
  console.info("[Unlock] simulation unlocked", { simulationId, shopDomain });
}
